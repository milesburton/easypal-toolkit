...
function crc8(data) {
  let crc = 0xff;
  for (const b of data) { crc ^= b; for (let i=0;i<8;i++) crc = crc&0x80?((crc<<1)^0xd5)&0xff:(crc<<1)&0xff; }
  return crc ^ 0xff;
}
function crc16(data) {
  let crc = 0xffff;
  for (const b of data) { crc ^= b<<8; for (let i=0;i<8;i++) crc = crc&0x8000?((crc<<1)^0x1021)&0xffff:(crc<<1)&0xffff; }
  return crc;
}
...
function popcount(x) {
  let n=x; n-=(n>>1)&0x55555555; n=(n&0x33333333)+((n>>2)&0x33333333);
  n=(n+(n>>4))&0x0f0f0f0f; return ((n*0x01010101)>>>24)&0xff;
}

function viterbiDecode(rxBits, puncture) {
  const K=7, numStates=CONV_STATES, rate=CONV_POLYNOMIALS.length;
  const bitsPerInput = puncture.filter(p=>p).length;
  const numInputBits = Math.floor(rxBits.length / bitsPerInput);
  let pm = new Float64Array(numStates).fill(1e9); pm[0]=0;
  const traceback = [];
  let rxIdx=0;
  for (let step=0;step<numInputBits;step++) {
    ...
    for (let s=0;s<numStates;s++) {
      ...
      for (const bit of [0,1]) {
        ...
        if (metric < newPm[nextState]) { newPm[nextState]=metric; tb[nextState]=s; }
      }
    }
    traceback.push(tb); pm=newPm; rxIdx+=bitsPerInput;
  }
  let best=0;
  ...
  for (let step=traceback.length-1;step>=0;step--) {
    ...
    if (prev===-1) { decoded.unshift(0); state=0; continue; }
...
function deserialiseSegments(data) {
  ...
  while (pos + MSC_SEGMENT_HEADER_BYTES + 2 <= data.length) {
    ...
    pos += 2;
    segments.push({ segNo, totalSegments, isLast: segNo===totalSegments-1, data: segData, crc });
  }
  return segments;
}
...
function reassembleMSC(segments, expectedTotal) {
  ...
  for (const seg of segments) {
    ...
    header[0]=(seg.segNo>>8)&0xff; header[1]=seg.segNo&0xff;
    header[2]=(seg.totalSegments>>8)&0xff; header[3]=seg.totalSegments&0xff;
    const combined = new Uint8Array(header.length+seg.data.length);
    combined.set(header); combined.set(seg.data, header.length);
  ...
  let offset=0;
  for (const seg of sorted) { result.set(seg.data,offset); offset+=seg.data.length; }
  return result;
}
...
function bitReverseFFT(re, im, n) {
  ...
  for (let i=0;i<n;i++) {
    let rev=0,x=i; for (let b=0;b<bits;b++){rev=(rev<<1)|(x&1);x>>=1;}
  ...
  for (let len=2;len<=n;len<<=1) {
    ...
    for (let i=0;i<n;i+=len) {
      ...
      for (let j=0;j<half;j++) {
        ...
        const tRe=twRe*re[v]-twIm*im[v],tIm=twRe*im[v]+twIm*re[v];
        re[v]=re[u]-tRe;im[v]=im[u]-tIm;re[u]+=tRe;im[u]+=tIm;
        const next=twRe*wRe-twIm*wIm;twIm=twRe*wIm+twIm*wRe;twRe=next;
      }
    }
  }
...
function buildInverse(perm) { const inv=new Array(perm.length); for (let i=0;i<perm.length;i++) inv[perm[i]]=i; return inv; }
...
function timeDeinterleave(cells,cols=30) {
  const n=cells.length,rows=Math.ceil(n/cols);
  ...
  for (let si=0;si<symbols.length;si++) {
    ...
    for (let ki=0;ki<NUM_CARRIERS;ki++) {
      ...
      if (!before&&after) H[si][ki]=[after.hRe,after.hIm];
        ...
        else { const t=(ki-before.ki)/(after.ki-before.ki); H[si][ki]=[before.hRe+t*(after.hRe-before.hRe),before.hIm+t*(after.hIm-before.hIm)]; }
      }
    }
  }
  return H;
...
function equalise(symbols,H) {
  return symbols.map((sym,si)=>sym.map(([rxRe,rxIm],ki)=>{
...
function coarseSync(samples) {
  const L=GUARD_SAMPLES,D=FFT_SIZE;
  const maxSearch=Math.min(samples.length-SYMBOL_SAMPLES,2*SYMBOL_SAMPLES);
  let bestCorr=-1,bestPos=0;
  for (let pos=0;pos<maxSearch;pos++) {
    let corrRe=0,eG=0,eD=0;
    for (let i=0;i<L;i++) {
      const g=samples[pos+i]??0,d=samples[pos+D+i]??0;
      corrRe+=g*d; eG+=g*g; eD+=d*d;
    }
    const norm=Math.sqrt(eG*eD);
    const mag=norm>1e-12?Math.abs(corrRe)/norm:0;
    if (mag>bestCorr){bestCorr=mag;bestPos=pos;}
  }
  return bestPos;
}
...
function decode(samples, inputSampleRate) {
  ...
  const allMSCBits=[],allFACBits=[],allSDCBits=[];
  for (const symbols of demodFrames) {
    ...
    for (const [sym,k] of FAC_CELLS) { const ki=k-K_MIN; allFACBits.push(...demap4QAM(eq[sym][ki][0],eq[sym][ki][1])); }
    for (const [sym,k] of SDC_CELLS) { const ki=k-K_MIN; allSDCBits.push(...demap4QAM(eq[sym][ki][0],eq[sym][ki][1])); }
  }
  ...
  for (let i=0;i<allSDCBits.length;i++) if (allSDCBits[i]) sdcBytes[i>>3]|=1<<(7-(i&7));
  ...
  for (let i=0;i<decodedBits.length;i++) if (decodedBits[i]) decodedBytes[i>>3]|=1<<(7-(i&7));

  const segments=deserialiseSegments(decodedBytes);
  console.log(`  Segments recovered: ${segments.length}`);

  const expectedSegs=sdcParams?.payloadBytes!==undefined
    ? Math.ceil(sdcParams.payloadBytes/(MSC_SEGMENT_BYTES-4)) : undefined;
  const reassembled=reassembleMSC(segments,expectedSegs);

  if (reassembled) {
    const isJpeg=reassembled.length>3&&reassembled[0]===0xff&&reassembled[1]===0xd8&&reassembled[2]===0xff;
