...
import { EASYPAL_MODES } from './EasyPalEncoder.js';

const FREQ_SYNC = 1200;
const FREQ_BLACK = 1500;
const FREQ_WHITE = 2300;

export interface EasyPalDecoderOptions {
  freqOffset?: number;
...
export class EasyPalDecoder {
  sampleRate: number;
  private mode: EasyPalMode | null;
  freqOffset: number;
  autoCalibrate: boolean;
  ...
  constructor(sampleRate: number = 48000, options: EasyPalDecoderOptions = {}) {
  ...
  private get currentMode(): EasyPalMode {
    if (this.mode === null) throw new Error('EasyPal mode not detected');
    return this.mode;
  }
  ...
  decodeSamples(samples: Float32Array): DecodeImageResult {
    ...
    if (!this.mode) {
      throw new Error('Could not detect EasyPal mode. Try manually selecting a mode.');
    }
  ...
  detectMode(samples: Float32Array): EasyPalMode {
    ...
    for (let i = 0; i < searchLimit - 1000; i += step) {
      ...
      if (Math.abs(freq - 1200) < 150) {
        ...
        for (const [_key, mode] of Object.entries(EASYPAL_MODES)) {
    ...
    this.visFreqShift = 0;
    return EASYPAL_MODES.ROBOT36 as EasyPalMode;
  }

  private detectModeByTiming(samples: Float32Array): EasyPalMode | null {
    ...
    for (const [, mode] of Object.entries(EASYPAL_MODES)) {
      const expected =
        mode.colorFormat === 'PD'
  ...
  decodeImage(
    ...
    if (!this.mode) throw new Error('No EasyPal mode set');
    ...
    if (position === -1) {
      throw new Error('Could not find sync pulse. Make sure this is a valid EasyPal transmission.');
    }
    ...
    if (mode.colorFormat === 'PD') {
      for (let y = 0; y < mode.lines; y += 2) {
        ...
        if (this.autoCalibrate && y + 2 < mode.lines) {
          const lineSamples = Math.floor((mode.componentTime ?? 0) * 4 * this.sampleRate);
  ...
  decodeScanLinePD(
    ...
    const mode = this.currentMode;
    const COMPONENT_TIME = mode.componentTime ?? 0;
    ...
    for (let x = 0; x < width; x++) {
      ...
      imageData.data[idx0] = y0values[x] ?? 0;
      imageData.data[idx0 + 1] = y0values[x] ?? 0;
      imageData.data[idx0 + 2] = y0values[x] ?? 0;
      imageData.data[idx0 + 3] = 255;

      const idx1 = (y1 * width + x) * 4;
      imageData.data[idx1] = y1values[x] ?? 0;
      imageData.data[idx1 + 1] = y1values[x] ?? 0;
      imageData.data[idx1 + 2] = y1values[x] ?? 0;
      imageData.data[idx1 + 3] = 255;

      chromaV[y * width + x] = ryValues[x] ?? 0;
      chromaV[y1 * width + x] = ryValues[x] ?? 0;
      chromaU[y * width + x] = byValues[x] ?? 0;
      chromaU[y1 * width + x] = byValues[x] ?? 0;
    }
