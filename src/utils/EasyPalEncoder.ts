import type { EasyPalMode } from '../types.js';

export const EASYPAL_MODES: Record<string, EasyPalMode> = {
  ROBOT36: {
    name: 'Robot 36',
    visCode: 0x08,
    scanTime: 0.15,
...
export class EasyPalEncoder {
  readonly mode: EasyPalMode;
  readonly sampleRate: number;
  private phase: number = 0;

  constructor(mode: string = 'ROBOT36', sampleRate: number = 48000) {
    const m = EASYPAL_MODES[mode];
    if (!m) throw new Error(`Unknown EasyPal mode: ${mode}`);
    this.mode = m;
